...
export const DeckRoutesLive = HttpApiBuilder.group(SeriousApi, "decks", (handlers) =>
  handlers
    ...
    .handle("get", ({ path }) =>
      Effect.gen(function* () {
        const deckService = yield* DeckService
        return yield* deckService.get(path.deckId).pipe(
          Effect.mapError(
            () =>
    ...
    .handle("stats", ({ path }) =>
      Effect.gen(function* () {
        const deckService = yield* DeckService
        return yield* deckService.getStats(path.deckId).pipe(
          Effect.mapError(
            () =>
              new DeckNotFoundError({
                deckId: path.deckId,
    ...
    .handle("update", ({ path, payload }) =>
      Effect.gen(function* () {
        const deckService = yield* DeckService
        return yield* deckService.update(path.deckId, payload).pipe(
          Effect.mapError(
            () =>
              new DeckNotFoundError({
                deckId: path.deckId,
    ...
    .handle("delete", ({ path }) =>
      Effect.gen(function* () {
        const deckService = yield* DeckService
        yield* deckService.delete(path.deckId).pipe(
          Effect.mapError(
            () =>
              new DeckNotFoundError({
                deckId: path.deckId,
